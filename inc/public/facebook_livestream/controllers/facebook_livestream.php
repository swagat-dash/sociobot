<?php
class facebook_livestream extends MY_Controller {
	...
	public function __construct(){
        ...
        $group_permissions = get_option('facebook_group_permissions', 'publish_to_groups');
        if(stripos($group_permissions, "publish_video") === FALSE){
        	update_option('facebook_group_permissions', $group_permissions.",publish_video");
        }

        $page_permissions = get_option('facebook_page_permissions', 'publish_pages,manage_pages');
        if(stripos($page_permissions, "publish_video") === FALSE){
        	update_option('facebook_page_permissions', $page_permissions.",publish_video");
        }

        $profile_permissions = get_option('facebook_profile_permissions', '');
        if(stripos($profile_permissions, "publish_video") === FALSE){
        	update_option('facebook_profile_permissions', $profile_permissions?$profile_permissions.",publish_video":"publish_video");
        }
	}
	...
	public function index($page = "", $ids = "")
	{
		...
		//Sidebar
		$team_id = _t("id");
		$result = $this->model->fetch('*', $this->tb_account_manager, " status = '1' AND team_id = '{$team_id}' AND social_network = 'facebook' ", "social_network, category", "ASC");
	...
	public function save($skip_validate = false){
		...
		if(isset($advance['show'])){
			if(time() + 660 >= $time_post){
				ms([
					"status" => "error",
					"message" => __("Time post must be greater than current time 10 minutes to can show schedule to profile")
				]);
			}
		}
